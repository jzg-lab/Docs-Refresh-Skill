...
- A taskified active plan is mandatory here: use `docs/exec-plans/active/<plan-slug>/index.md` plus one markdown file per execution phase in the same plan directory, and follow [references/execution-plan-contract.md](../references/execution-plan-contract.md) for the required task schema. Chapter-only plans are not execution-ready.
