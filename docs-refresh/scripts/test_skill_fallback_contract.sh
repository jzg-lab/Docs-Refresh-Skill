...
assert_contains "$skill_dir/SKILL.md" 'Actionable execution planning requires a taskified active plan with one markdown file per execution phase'
...
assert_contains "$skill_dir/SKILL.md" 'split it into a plan index plus phase-scoped markdown files under the active plan directory'
...
assert_contains "$skill_dir/modes/structured.md" 'A taskified active plan is mandatory here: use one markdown file per execution phase'
...
assert_contains "$skill_dir/references/foundation-checklist.md" 'prefer a plan index plus one markdown file per execution phase'
...
assert_contains "$skill_dir/references/execution-plan-contract.md" 'Use a plan index plus one markdown file per execution phase under the active plan directory.'
...
assert_contains "$skill_dir/agents/openai.yaml" 'materialize an active plan index plus one markdown file per execution phase'
assert_contains "$skill_dir/agents/openai.yaml" 'require checklist-grade execution tasks with preconditions, files to change, expected output, done-when criteria, blockers, and dependency or parallelism notes inside each phase file'
assert_contains "$skill_dir/agents/openai.yaml" 'split vague single-document execution plans into phase files when the phase boundaries are reliable'
...
assert_contains "$repo_root/README.md" 'plan index plus one markdown file per execution phase'
assert_contains "$repo_root/README.md" 'materialize the planning scaffold, directory placement, an active plan index, and one markdown file per execution phase instead of replying with advice only'
assert_contains "$repo_root/README.md" 'require execution-ready phase files to use checklist-grade tasks with preconditions, files to change, expected outputs, done-when criteria, blockers, and dependency or parallelism notes'
assert_contains "$repo_root/README.md" 'split it into phase files when the phase boundaries are reliable'
