...
assert_contains "$skill_dir/SKILL.md" 'Path semantics and content semantics must agree unless the repository explicitly documents a different lifecycle convention.'
assert_contains "$skill_dir/SKILL.md" 'Shared baseline, prerequisite, or common acceptance plans follow the same lifecycle rule'
assert_contains "$skill_dir/SKILL.md" 'Treat `completed in content, still in active/` as a lifecycle inconsistency to fix in the same round'
...
assert_contains "$repo_root/README.md" 'later active phases should link to the stable `completed/` path instead of keeping the old placement alive'
