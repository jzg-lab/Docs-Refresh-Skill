...
- If user intent is materially ambiguous and the ambiguity would change routing, ask concise clarifying questions before you choose a mode or update durable docs.
...
- Treat the collector's `plan_readiness` as a planning-surface signal, not as proof that a specific plan document is execution-ready. A ready scaffold does not make a chapter-only plan acceptable.
- Actionable execution planning requires a taskified active plan with one markdown file per execution phase under `docs/exec-plans/active/<plan-slug>/`, not just chapter headings or `Step 1/2/3` prose inside a single document. Follow [references/execution-plan-contract.md](references/execution-plan-contract.md).
- When building an active execution plan, if scope, constraints, dependencies, sequencing, or acceptance criteria are materially unclear, ask the user concise clarifying questions before taskifying the plan.
- Do not fabricate executable steps from unresolved ambiguity. If key decisions remain open after clarification, mark the work as not execution-ready and persist the blockers in durable docs instead of drafting a fake active plan.
- If an existing planning document is a single vague narrative but the work is now execution-ready, split it into a plan index plus phase-scoped markdown files under `docs/exec-plans/active/<plan-slug>/`. If you cannot determine reliable phase boundaries from current docs and explicit user intent, ask before you split.
...
- Plan lifecycle: In structured repos with a standard `docs/exec-plans/` tree, audit `stale_plan_placement` even when git is clean. Move done-marked legacy plan files and done-marked `active/<plan-slug>/` plan directories into `completed/` in the same pass, including shared baseline or prerequisite plans once they are accepted. A completed phase file inside an otherwise active plan is not stale by itself. Update indexes, umbrella docs, and cross-links so path semantics and content semantics stay aligned. Treat `completed in content, still in active/` as a lifecycle inconsistency only when the whole plan is complete but still lives in `active/`.
