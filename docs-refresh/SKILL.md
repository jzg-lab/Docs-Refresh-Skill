...
- Actionable execution planning requires a taskified active plan with one markdown file per execution phase, not just chapter headings or `Step 1/2/3` prose inside a single document. Follow [references/execution-plan-contract.md](references/execution-plan-contract.md).
...
- If an existing planning document is a single vague narrative but the work is now execution-ready, split it into a plan index plus phase-scoped markdown files under the active plan directory. If you cannot determine reliable phase boundaries from current docs and explicit user intent, ask before you split.
