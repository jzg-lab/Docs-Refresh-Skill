...
- If the collector reports `doc_system_mode=repair`, or non-empty `missing_index_targets`, `missing_plan_scaffold_targets`, or `repair_targets`, that repair work is mandatory. Do not treat empty `doc_review_triggers` or a conservative `doc_refresh_hint` as permission to skip the repair.
