...
5. If the collector succeeds, read both `[routing]` and `[knowledge]`, open only the matching mode file from `preferred_mode_doc`, and load [references/foundation-checklist.md](references/foundation-checklist.md) when `knowledge_phase` is not `operations` or `foundation_gaps` is non-empty.
...
- If explicit planning intent targets `docs/exec-plans/` and the default scaffold is missing, create the minimum scaffold needed and land the active plan in the same pass instead of stopping at repair alone.
