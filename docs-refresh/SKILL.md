...
- In structured repos with lifecycle directories (`active/`, `completed/`), also audit for internal consistency: index marks vs file locations, stale plan files in wrong lifecycle buckets, and broken cross-links. These drift silently and are not caught by diff-driven rules.
