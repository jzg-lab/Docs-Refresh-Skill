...
1. `docs-refresh/SKILL.md` is the authoritative workflow and behavior contract.
2. `docs-refresh/scripts/collect_changed_context.sh` is the authoritative definition of the bundled context collector.
3. `docs-refresh/agents/openai.yaml` is optional adapter metadata for OpenAI-compatible surfaces and should stay aligned with the workflow name and short description.
4. `README.md` is the human-facing overview and install pointer.
...
- Update `docs-refresh/SKILL.md` first when behavior changes.
